...
func (r Running) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		TrainingType: runInfo.TrainingType,
...
func (w Walking) Calories() float64 {
	...
	speedMsec = math.Pow(speedMsec, 2)
	heightM := w.Height / CmInM
	// вставьте ваш код ниже
	return ((CaloriesWeightMultiplier*w.Weight +
		(speedMsec/heightM)*CaloriesSpeedHeightMultiplier*w.Weight) *
...
func (w Walking) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	walkingInfo := w.Training.TrainingInfo()
	return InfoMessage{
		TrainingType: walkingInfo.TrainingType,
		Duration:     walkingInfo.Duration,
		Distance:     walkingInfo.Distance,
		Speed:        walkingInfo.Speed,
		Calories:     walkingInfo.Calories,
	}
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	return float64(s.LenghtPool*s.CountPool) / MInKm / s.Duration.Hours()
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     s.distance(), // пересчёт дистанции
		Speed:        s.meanSpeed(),
		Calories:     s.Calories(),
	}
}
